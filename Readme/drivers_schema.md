# Drivers Schema Conversion

## Overview
This document describes the schema conversion for the [drivers](file:///Users/mac-1/Desktop/order_fulfillment/routers/driversUtility.py#98-160) table from MySQL to PyIceberg and PyArrow formats.

## MySQL Schema Highlights

- **Varchar Primary Keys**: `id` is `varchar(100)`, not integer.
- **JSON Fields**: `branch_code_list`, `branchcode`, `roles` are stored as JSON strings.
- **Boolean Fields**: `isActive` is `tinyint(1)`.
- **Timestamps**: Multiple creation/update timestamps (`created_on`, `created_at`, `createdAt`).
...
```python
from routers.driversUtility import drivers_schema
...
iceberg_schema, arrow_schema = drivers_schema(sample_record)
```

### 2. Clean Data

```python
from routers.driversUtility import drivers_clean_rows

raw_rows = [
    {
        "id": "DRV001",
        "pincode": "500001",  # String to Int
...
cleaned_rows = drivers_clean_rows(raw_rows)
```

## File Location
- **Utility File**: [driversUtility.py](file:///Users/mac-1/Desktop/order_fulfillment/routers/driversUtility.py)
