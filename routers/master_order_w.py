...
# multithreading
# mysql | range | chunk_size | iceberg | arrow | append
...
def insert(
    ...
    try:
        ...
        rows = mysql_creds.get_master_order_w(dbname, start_range, end_range,"2025-12-12")
...
def multi_within_mysql_date_range(
    ...
    try:
        ...
        rows = mysql_creds.get_master_w_order_date_range(dbname, start_dt, end_dt)
...
def insert_pickup_delivery_items(
    start_range: str = Query(0, description="Start row offset for MySQL data fetch"),
    end_range: str = Query(100, description="End row offset for MySQL data fetch"),
):
    total_start = time.time()
    namespace, table_name = "order_fulfillment", "masterorders_w"
    dbname = "masterorders_w"
    ...
    try:
        rows = mysql_creds.get_master_w_order_date_range(dbname, start_range, end_range)
        if not rows:
            raise HTTPException(status_code=400, detail="No data found in the given range.")
