...
from .installation_servicesUtility import *
...
logger = get_logger("installation_services")

router = APIRouter(prefix="/installation_services", tags=["installation_services"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_installation_services_between_range(
    ...
    namespace, table_name = "order_fulfillment", "installation_services"
    dbname = "installation_services"
    ...
    try:
        ...
        rows = mysql_creds.get_installation_services(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from installation_services")
    ...
    try:
        # print(rows[0])
        installation_services_clean_rows(rows)
    ...
    iceberg_schema, arrow_schema = installation_services_schema(rows[0])
