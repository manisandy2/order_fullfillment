...
from .exchange_masterordersUtility import *
...
logger = get_logger("exchange_masterorders")

router = APIRouter(prefix="/exchange_masterorders", tags=["exchange_masterorders"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_exchange_masterorders_between_range(
    ...
    namespace, table_name = "order_fulfillment", "exchange_masterorders"
    dbname = "exchange_masterorders"
    ...
    try:
        ...
        rows = mysql_creds.get_exchange_masterorders(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from exchange_masterorders")
    ...
    try:
        exchange_masterorders_clean_rows(rows)
    ...
    iceberg_schema, arrow_schema = exchange_masterorders_schema(rows[0])
