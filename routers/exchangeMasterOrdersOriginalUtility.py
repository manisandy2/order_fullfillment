...
JSON_FIELDS = [
    ...
    "service_details", "lineitems", "lineitem_status"
]
...
FIELD_OVERRIDES = {
    ...
    "invoice_reff_date": (StringType(), pa.string(), False), # text
    ...
    "invoice_pdf": (StringType(), pa.string(), False), # text
...
def exchange_masterorders_original_schema(record: Dict[str, Any]) -> Tuple[Schema, pa.Schema]:
    """
    Generate Iceberg and Arrow schemas for exchange_masterorders table (Original).
...
def exchange_masterorders_original_clean_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Clean and normalize row data for exchange_masterorders (Original) schema compliance.
    ...
    for row in rows:
        ...
        for f in DECIMAL_FIELDS:
            ...
            if isinstance(val, str):
                ...
                # Default for this table is 0.000000 in schema, but nullable=True.
                # Usually better to default to 0.0 if that's the business logic for coordinates.
                row[f] = 0.0
