...
logger = get_logger("intransit_manifests")

router = APIRouter(prefix="/intransit_manifests", tags=["intransit_manifests"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_intransit_manifests_between_range(
    ...
    namespace, table_name = "order_fulfillment", "intransit_manifests"
    ...
    try:
        ...
        rows = mysql_creds.get_intransit_manifests(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_intransit_manifests")
    ...
    try:
        # print(rows[0])
        intransit_manifests_clean_rows(rows)
    ...
    iceberg_schema, arrow_schema = intransit_manifests_schema(rows[0])
