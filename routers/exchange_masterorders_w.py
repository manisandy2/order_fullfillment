...
logger = get_logger("exchange_masterorders_w")

router = APIRouter(prefix="/exchange_masterorders_w", tags=["exchange_masterorders"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_exchange_masterorders_w_between_range(
    ...
    namespace, table_name = "order_fulfillment", "exchange_masterorders_w"
    dbname = "exchange_masterorders_w"
    ...
    try:
        ...
        rows = mysql_creds.get_exchange_masterorders_w(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from exchange_masterorders_w")
    ...
    try:
        exchange_masterorders_w_clean_rows(rows)
    ...
    iceberg_schema, arrow_schema = exchange_masterorders_w_schema(rows[0])
