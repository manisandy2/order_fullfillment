...
from .pickup_deliveriesUtility import *
...
logger = get_logger("pickup_deliveries")

router = APIRouter(prefix="/pickup_deliveries", tags=["pickup_deliveries"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_pickup_deliveries_between_range(
    ...
    namespace, table_name = "order_fulfillment", "pickup_deliveries"
    ...
    try:
        ...
        rows = mysql_creds.get_pickup_deliveries(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_pickup_deliveries")
