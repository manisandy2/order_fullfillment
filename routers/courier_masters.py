...
logger = get_logger("courier_masters-api")

router = APIRouter(prefix="", tags=["courier_masters"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/courier_masters/ingest/mysql-range")
def courier_masters_between_range(
    ...
    namespace, table_name = "order_fulfillment", "courier_masters"
    ...
    try:
        ...
        rows = mysql_creds.get_courier_masters(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from courier_masters")
    ...
    try:
        courier_masters_clean_rows(rows)
    ...
    iceberg_schema, arrow_schema = courier_masters_schema(rows[0])
