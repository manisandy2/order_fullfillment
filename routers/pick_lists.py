...
logger = get_logger("pick_lists")

router = APIRouter(prefix="/pick_lists", tags=["pick_lists"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_pick_lists_between_range(
    ...
    namespace, table_name = "order_fulfillment", "pick_lists"
    dbname = "pick_lists"
    ...
    try:
        ...
        rows = mysql_creds.get_pick_lists(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_pick_lists")
