...
logger = get_logger("intransit_pickup_delivery_items")

router = APIRouter(prefix="/intransit_pickup_delivery_items", tags=["intransit_pickup_delivery_items"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_intransit_pickup_delivery_items_between_range(
    ...
    namespace, table_name = "order_fulfillment", "intransit_pickup_delivery_items"
    dbname = "intransit_pickup_delivery_items"
    ...
    try:
        ...
        rows = mysql_creds.get_intransit_pickup_delivery_items(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_intransit_pickup_delivery_items")
