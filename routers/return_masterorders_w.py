...
from .return_masterorders_wUtility import *
...
logger = get_logger("return_masterorders_w")

router = APIRouter(prefix="/return_masterorders_w", tags=["return_masterorders_w"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_return_masterorders_w_between_range(
    ...
    namespace, table_name = "order_fulfillment", "return_masterorders_w"
    dbname = "return_masterorders_w"
    ...
    try:
        ...
        rows = mysql_creds.get_return_masterorders_w(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_return_masterorders_w")
