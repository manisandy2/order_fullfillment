...
from .return_orderlineitemsUtility import *
...
logger = get_logger("return_orderlineitems")

router = APIRouter(prefix="/return_orderlineitems", tags=["return_orderlineitems"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_return_orderlineitems_between_range(
    ...
    namespace, table_name = "order_fulfillment", "return_orderlineitems"
    dbname = "return_orderlineitems"
    ...
    try:
        ...
        rows = mysql_creds.get_return_orderlineitems(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_return_orderlineitems")
