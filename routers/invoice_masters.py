...
logger = get_logger("invoice_masters")

router = APIRouter(prefix="/invoice_masters", tags=["invoice_masters"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_invoice_masters_between_range(
    ...
    namespace, table_name = "order_fulfillment", "invoice_masters"
    ...
    try:
        ...
        rows = mysql_creds.get_invoice_masters(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_invoice_masters")
