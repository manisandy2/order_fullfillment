...
# from .exchange_informationsUtility import *
from .exchange_orderlineitemsUtility import *
...
logger = get_logger("exchange_orderlineitems")

router = APIRouter(prefix="/exchange_orderlineitems", tags=["exchange_orderlineitems"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_exchange_orderlineitems_between_range(
    ...
    namespace, table_name = "order_fulfillment", "exchange_orderlineitems"
    dbname = "exchange_orderlineitems"
    ...
    try:
        ...
        rows = mysql_creds.get_exchange_orderlineitems(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_exchange_orderlineitems")
    ...
    try:
        # print(rows[0])
        exchange_orderlineitems_clean_rows(rows)
        # print(exchange_orderlineitems_clean_rows(rows))
        # exchange_informations_clean_rows(rows)
    ...
    iceberg_schema, arrow_schema = exchange_orderlineitems_schema(rows[0])
