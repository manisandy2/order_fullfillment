...
from .reason_messagesUtility import *
...
logger = get_logger("reason_messages")

router = APIRouter(prefix="/reason_messages", tags=["reason_messages"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_reason_messages_between_range(
    ...
    namespace, table_name = "order_fulfillment", "reason_messages"
    dbname = "reason_messages"
    ...
    try:
        ...
        rows = mysql_creds.get_reason_messages(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_reason_messages")
