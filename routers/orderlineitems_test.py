...
logger = get_logger("orderlineitems-test-api")

router = APIRouter(prefix="", tags=["Order Line Items"])
# Order Line Items
# multithreading
@router.post("/orderlineitems_test/insert-multi-with-mysql")
def multi_within_mysql(
    ...
    try:
        rows = mysql_creds.get_orderlineitems_test(dbname, start_range, end_range)
        print("rows =", rows[0])
        print("Data:",len(rows))
    ...
    try:
        orderlineitems_test_clean_rows(rows)
