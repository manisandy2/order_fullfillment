...
from .return_masterordersUtility import *
...
logger = get_logger("return_masterorders")

router = APIRouter(prefix="/return_masterorders", tags=["return_masterorders"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_return_masterorders_between_range(
    ...
    namespace, table_name = "order_fulfillment", "return_masterorders"
    dbname = "return_masterorders"
    ...
    try:
        ...
        rows = mysql_creds.get_return_masterorders(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_return_masterorders")
