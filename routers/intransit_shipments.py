...
from .intransit_shipmentsUtility import *
...
logger = get_logger("intransit_shipments")

router = APIRouter(prefix="/intransit_shipments", tags=["intransit_shipments"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_intransit_shipments_between_range(
    ...
    namespace, table_name = "order_fulfillment", "intransit_shipments"
    dbname = "intransit_shipments"
    ...
    try:
        ...
        rows = mysql_creds.get_intransit_shipments(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_intransit_shipments")
