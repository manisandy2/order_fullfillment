...
logger = get_logger("drivers_dob_error")

router = APIRouter(prefix="", tags=["drivers_dob_error"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/drivers_dob_error/ingest/mysql-range")
def drivers_between_range(
    ...
    namespace, table_name = "order_fulfillment", "drivers_dob_error"
    ...
    try:
        ...
        rows = mysql_creds.get_drivers_dob_error(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from get_drivers_dob_error")
    ...
    # iceberg_schema, arrow_schema = drivers_schema(rows[0])
