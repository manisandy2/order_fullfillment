...
logger = get_logger("hub_masters")

router = APIRouter(prefix="/hub_masters", tags=["hub_masters"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_hub_masters_between_range(
    ...
    namespace, table_name = "order_fulfillment", "hub_masters"
    dbname = "hub_masters"
    ...
    try:
        ...
        rows = mysql_creds.get_hub_masters(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from hub_masters")
    ...
    iceberg_schema, arrow_schema = hub_masters_schema(rows[0])
