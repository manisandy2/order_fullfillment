...
logger = get_logger("externalcalllogs")

router = APIRouter(prefix="/externalcalllogs", tags=["externalcalllogs"])


# mysql | range | chunk_size | multithreading | arrow | append
@router.post("/ingest/mysql-range")
def insert_externalcalllogs_between_range(
    ...
    namespace, table_name = "order_fulfillment", "externalcalllogs"
    dbname = "externalcalllogs"
    ...
    try:
        ...
        rows = mysql_creds.get_externalcalllogs(dbname, start_range, end_range)
        print(f"{len(rows)} rows fetched from externalcalllogs")
    ...
    iceberg_schema, arrow_schema = external_call_logs_schema(rows[0])
